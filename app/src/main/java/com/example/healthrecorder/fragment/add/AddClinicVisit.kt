...
class AddClinicVisit : Fragment() {
    ...
    override fun onCreateView(
        ...
        view.bkbutton400.setOnClickListener {
            findNavController().navigate(AddClinicVisitDirections.actionAddClinicVisitToListClinicVisit())
        }
            ...
            when {
                ...
                TextUtils.isEmpty(add_time.text?.trim() { it <= ' ' }) -> {
                    ...
                    val clinicVisit = ClinicVisit(
                        ...
                        add_time.text.toString(), add_clinic_name.text.toString()
                    )
                    mFireStore.collection("users").document(currentUser.uid)
                        ...
                        .add(clinicVisit).addOnSuccessListener { documentReference ->
                            ...
                            mFireStore.collection("users").document(currentUser.uid)
                                .collection("Clinic Visit").document("$document")
                                .update("id", "$document")
                        }
